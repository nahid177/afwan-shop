...
const CreateOrUpdateCarousel: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation:
    // - In create mode (no id), name and imageFile are required
    // - In update mode, name is required; imageFile is optional
    if (!name || (!id && !imageFile)) {
      toast.error('Carousel name is required' + (!id ? ' and image are required' : ''));
      return;
    }
    ...
    try {
      let uploadedImageUrl = imageUrl; // Default to existing imageUrl
      ...
      setLink('');
      setImageFile(null);
      setImageUrl(''); // Reset imageUrl after creation
        ...
        <form onSubmit={handleSubmit} className="max-w-lg mx-auto bg-white p-8 rounded-xl shadow-lg">
          ...
          <div className="mb-6">
            <label htmlFor="imageFile" className="block text-sm font-medium text-gray-700">
              {id ? 'Change Image (optional)' : 'Upload Image'}
            </label>
