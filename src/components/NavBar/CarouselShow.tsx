...
const CarouselShowIconshow = () => {
  const [iconshowItems, setIconshowtems] = useState<IconshowItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [editableItem, setEditableItem] = useState<IconshowItem | null>(null);
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await axios.delete(`/api/iconshow/delete/${id}`);
      if (response.status === 200) {
        setIconshowtems(iconshowItems.filter(item => item._id !== id));
  ...
  // Handle edit action
  const handleEdit = (item: IconshowItem) => {
  ...
  const handleSaveEdit = async () => {
    if (editableItem) {
      try {
        const response = await axios.put(`/api/iconshow/update/${editableItem._id}`, editableItem);
        if (response.status === 200) {
          setIconshowtems(
            iconshowItems.map(item =>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8 ">
      <h1 className="text-4xl font-bold mb-6 text-center">Iconshow Items</h1>
...
export default CarouselShowIconshow;
